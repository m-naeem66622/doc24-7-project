...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    patientId = req.generatedId;

    cb(null, "public/patients/" + patientId + "/");
  },
  filename: (req, file, cb) => {
    try {
      ...
      const fullFilePath = `./public/patients/${patientId}/${file.originalname}`;
      ...
      fs.mkdirSync("public/patients/" + patientId + "/", { recursive: true });
      ...
      console.log("error in patient profile upload", error);
    }
  },
});
...
/* INITIALIZING MULTER */
const uploadPatientProfile = multer({ storage, limits, fileFilter });

module.exports = uploadPatientProfile;
